...
help()
{
	...
	echo ""
	echo "Citation:"
	echo "Gerber et al. 2022 Interdisciplinary analyses of Bronze age communities from Western Hungary reveal complex population histories"
	echo ""
	echo "Options:"
...
do
	case "${opts}" in
		...
		g) GUI=true; if ! command -v yad &> /dev/null; then echo "yad is not installed, try sudo apt install yad -y, also check dependencies"; else ./src/papline_gui.sh; fi;;
		h) help; exit 2;;
