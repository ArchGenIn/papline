...
if [ $mtfas == Intelligent ] || [ $mtfas == Majority_rule ] || [ $mtfas == Random ] || [ $disis != 0 ] || [ $pigme != 0 ] || [ $glqua != 0 ]; then
...
if [ $presq == TRUE ] || [ $presq == true ] || [ $presq == T ] || [ $presq == t ]; then
	if ! command -v preseq &> /dev/null; then
		echo "Preseq is not installed despite presq=TRUE, check dependencies"
		exit 1
	fi
fi
...
if ! command -v bamtools &> /dev/null; then
	...
	echo "SAMtools is not installed, try sudo apt install samtools -y, also check dependencies and don't afraid to ask someone for some help, it seems that you are at the beginning of this wonderful journey"
	exit 1
	...
	if [ $fqc != 0 ] || [ $fac != 0 ] || [ $fqg != 0 ] || [ $fag != 0 ]; then
		...
		if [ ! -z $align ]; then
			if [ $align == BWA_ALN ] || [ $align == BWA_MEM ]; then
				...
				befer=$( echo "${refer%.*}" )
				if [ ! -f "$befer".1.bt* ]; then
					echo "Bowtie2 reference does not exists or improperly formatted despite selecting Bt2 as aligner method, check your input or PARAMETERFILE"
					exit 1
				fi
			else
